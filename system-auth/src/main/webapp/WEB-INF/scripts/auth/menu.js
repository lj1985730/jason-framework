!function ($) {
    ...
    $(function() {
        ...
        $("#enableMenu").on("click", function () {
            var selectMenus = $tree.treeSelect('getData');
            ...
            $.fn.http({
                method : 'PUT',
                url : baseUrl + '/' + selectMenus[0].id + '/toggleEnable',
                success : function() {
                    SysMessage.alertSuccess('菜单"' + selectMenus[0].text + '"状态已变更！');
                    loadTreeData();
                }
            });
        });
        $('#createMenu').on('click', function() { create(); });
        $('#deleteMenu').on('click', function() { remove(); });
